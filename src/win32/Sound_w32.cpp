...
DIRSOUND::DIRSOUND(HWND hwnd)
{
  ...
  if (ret != DS_OK) {
    InfoChip8_MessageBox( "Sound Card is needed to execute this application." );
    exit(-1);
  }
  ...
  if ( ret != DS_OK ) {
    InfoChip8_MessageBox( "SetCooperativeLevel() Failed." );
    exit(-1);
  }
}
...
void DIRSOUND::FillBuffer( WORD channel )
{
  ...
  if (hr != DS_OK) {
    InfoChip8_MessageBox( "Lock() Failed." );
    exit(-1);
  }
  ...
  if (hr != DS_OK) {
    InfoChip8_MessageBox( "Unlock() Failed." );
    exit(-1);
  }
}
...
void DIRSOUND::CreateBuffer(WORD channel)
{
  ...
  if (hr != DS_OK) {
    InfoChip8_MessageBox( "CreateSoundBuffer() Failed." );
    exit(-1);
  }
}
...
void DIRSOUND::DestroyBuffer(WORD channel)
{
  ...
  if (hr != DS_OK) {
    InfoChip8_MessageBox( "Release() Failed." );
    exit(-1);
  }
...
WORD DIRSOUND::AllocChannel(void)
{
  ...
  if ( x == ds_NUMCHANNELS ) {
    ...
    InfoChip8_MessageBox( "AllocChannel() Failed." );
    exit(-1); 		
  }
...
void DIRSOUND::Start(WORD channel, BOOL looping)
{
  ...
  if ( hr != DS_OK ) {
    InfoChip8_MessageBox( "Play() Failed." );
    exit(-1);
  }
}
...
BOOL DIRSOUND::SoundOpen(int samples_per_sync, int sample_rate)
{
  ...
  if ( sound[ch1] == NULL ) {
    InfoChip8_MessageBox( "new BYTE[] Failed." );
    exit(-1);
  }
  CreateBuffer( ch1 );
